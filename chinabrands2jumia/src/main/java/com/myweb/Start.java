...
public class Start {
    ...
    private int expires;
    private CBAPIKey cbapiKey;

    public Start() {
        try {
            File apiFile = new File("C:\\C2J\\SKU\\cbapiKey.xlsx");
            CBAPIKey apiKey = ObjectExcelRead.readCBAPIKeyByFileForXlsx(apiFile, 0);
            if (apiKey != null) {
                System.out.println("C:\\C2J\\SKU\\cbapiKey.xlsx文件读取CBAPIKEY数据成功！");
                cbapiKey = apiKey;
            } else {
                System.out.println("C:\\C2J\\SKU\\cbapiKey.xlsx文件读取CBAPIKEY数据失败！");
            }
        } catch (Exception e) {
            System.out.println("C:\\C2J\\SKU\\cbapiKey.xlsx文件读取CBAPIKEY数据失败！");
        }
    ...
    public void StartInit() {
        ...
        if (cbapiKey != null) {
            try {
                ...
                String tokenString = chinaBrandsAPI.getToken(cbapiKey.getEmail(), cbapiKey.getPassword(), cbapiKey.getClientid(), cbapiKey.getApikey());
                ...
                if (tokenResult.getStatus() == 1) {
                    ...
                    System.out.println("账号为"+cbapiKey.getEmail()+"的ChinaBrands API 授权成功！！！！");
                } else {
                    System.out.println("账号为"+cbapiKey.getEmail()+"的ChinaBrands API 授权失败！！！！");
                }
            } catch (Exception e) {
                System.out.println("账号为"+cbapiKey.getEmail()+"的ChinaBrands API 授权失败！！！！");
            }
    ...
    public static void main(String[] args) {
        ...
        start.StartInit();
        if(start.getCbapiKey() == null || start.getToken() == null || start.getExpires() == 0){
            System.out.println("ChinaBrands API初始化失败！！！！");
            return;
        }
    ...
    public CBAPIKey getCbapiKey() {
        return cbapiKey;
    }

    public void setCbapiKey(CBAPIKey cbapiKey) {
